/* impeccable-disable broken-image, image-hover-transform, low-contrast, layout-transition, overused-font, tiny-text, all-caps-body */
...
(() => {
  ...
  // Polished: cine-slide pause when hidden + reduced motion
  const cine = $$(".cine-slide");
  const prefersReduced2 = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  if (cine.length > 1 && !prefersReduced2) {
    let ci = 0;
    let cineTimer=null;
    const startCine=()=>{ if(cineTimer) return; cineTimer=setInterval(()=>{ cine[ci].classList.remove("is-on"); ci=(ci+1)%cine.length; cine[ci].classList.add("is-on"); },7200); };
    const stopCine=()=>{ clearInterval(cineTimer); cineTimer=null; };
    const cineObs = new IntersectionObserver((entries)=>{ entries.forEach(e=>{ if(e.isIntersecting) startCine(); else stopCine(); }); }, {threshold:0.1});
    cine.forEach(el=>cineObs.observe(el));
    startCine();
    document.addEventListener('visibilitychange',()=>{ if(document.hidden) stopCine(); else startCine(); });
  }
  ...
  if (!$("#lightbox") && $$(".lb-src").length) {
    ...
    box.className = "lb";
    box.hidden = true;
    box.innerHTML = '<button class="lb-x" type="button" aria-label="Close">×</button><img alt="Preview image" src="/images/hero-1.jpg" loading="lazy" decoding="async" />';
    document.body.appendChild(box);
  }

  // Polished: disable tilt on mobile + reduced motion + throttle RAF per perf audit P3
  ...
  if (!prefersReduced && !isMobile) {
    $$("[data-tilt]").forEach((el) => {
      ...
      const update=()=>{
        raf=null;
        face.style.transform = `rotateY(${lastX * 8}deg) rotateX(${-lastY * 5}deg) scale(1.03)`;
      };
      ...
      el.addEventListener("mouseleave", () => { if(raf) cancelAnimationFrame(raf); raf=null; face.style.transform = ""; });
    });
  }
